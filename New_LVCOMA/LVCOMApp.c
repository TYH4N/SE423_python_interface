...
int main (int argc, char **argv)
{
	...
	printf(".\n");
	printf("espeak \"Initializing!\"");
	size_buff = sprintf(buffer4SystemCall,"espeak \"It's Gamer Time!\"");//\"Initializing!\"");
	if (size_buff < 512) {
	 	system(buffer4SystemCall);
	}
	...
	while (!gs_exit) {
		...
		if (sem_trywait(sendto_LVCOMApp_mutex_sem) == 0) {
			...
			if (f28xtoLVFloat[6] == 1.0) {
				size_buff = sprintf(buffer4SystemCall,"espeak \"Orange Detected!\"");
				if (size_buff < 512) {
				 	system(buffer4SystemCall);
				}
					  
			} else if (f28xtoLVFloat[6] == 2.0) {
				size_buff = sprintf(buffer4SystemCall,"espeak \"Purple Detected!\"");
				if (size_buff < 512) {
				 	system(buffer4SystemCall);
				}
			} else if (f28xtoLVFloat[6] == 3.0) {
				size_buff = sprintf(buffer4SystemCall,"espeak \"Orange Secured!\"");
				if (size_buff < 512) {
				 	system(buffer4SystemCall);
				}
			} else if (f28xtoLVFloat[6] == 4.0) {
				size_buff = sprintf(buffer4SystemCall,"espeak \"Purple Secured!\"");
				if (size_buff < 512) {
				 	system(buffer4SystemCall);
				}
			} else if (f28xtoLVFloat[6] == 5.0) {
				size_buff = sprintf(buffer4SystemCall,"espeak \"Packages Delivered!\"");
				if (size_buff < 512) {
				 	system(buffer4SystemCall);
				}
			} else if (f28xtoLVFloat[6] == 6.0) {

				size_buff = sprintf(buffer4SystemCall,"espeak \"Purple Delivered!\"");
				if (size_buff < 512) {
				 	system(buffer4SystemCall);
				}
			} else if (f28xtoLVFloat[6] == 7.0) {
				size_buff = sprintf(buffer4SystemCall,"python play_music.py");
				if (size_buff < 512) {
				 	system(buffer4SystemCall);
				}
